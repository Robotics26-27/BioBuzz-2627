 ...
 *
 * Paired with FieldCentricTeleOpPedro, which does the same job through the follower and
 * is about half the length. The reason to keep this version: it needs nothing but four
 * motors and the hub, so it drives before Constants.java has ever been tuned. Once the
 * follower is tuned, prefer the Pedro one -- it reuses tuned gains and there is no hand
 * written wheel mixing to get a sign wrong in.
 *
 * Heading comes from the IMU alone here, so it drifts over a match and resetYaw is the
 * only cure. The Pedro version reads heading from the localizer instead.
 */
@TeleOp(name = "Example: Field Centric (IMU)", group = "Examples")
public class FieldCentricTeleOpImu extends OpMode {
